...
def extract_product_data(product, category):
    try:
        ...
        product_name = product.find('h2', class_='card-title h4').text.strip()

        if product_name == "Hoodie combo (Unisex)":
            print(f"Skipping product: {product_name}")
            return None

        product_link = product.find('a', class_='no-style').get('href')
        img_container = product.find('div' , class_='primary')
        img_tag = img_container.find('img')
        primary_image = img_tag['src'].lstrip('//') if img_tag else "No image available"

        price_container = product.find('x-cell', class_='card-price')
        original_price = price_container.find('span', class_='new-price')
        if original_price:
    ...
    return {
        ...
        'Product Link': "https://gullylabs.com" + product_link,
...
def extract_detailed_product_info(product_link):
    ...
    try:
        ...
        description_container = soup.find('div', class_='text rte')
        second_p_tag = description_container.find_all('p')[1] if description_container else None
        if second_p_tag:
            description = second_p_tag.get_text(strip=True)
        # Adjusting the size options selector
        sizes_container = soup.find('ul', class_='no-style swatch-group')
        sizes = []
        if sizes_container:
            ...
            for input_tag in size_inputs:
                if input_tag.has_attr('value'):
                    ...
                    label_tag = sizes_container.find('label', {'for': input_tag['id']})
                    if label_tag:
                        sizes.append(label_tag.get_text(strip=True))

        # Print the extracted sizes
        print(sizes)
        # Extracting images from each product-image-main container
        images_container = soup.find_all('x-cell', class_='slider__slide')
        all_images = []
...
def scrape_products(category_url, category, writer):
    ...
    while True:
        ...
        soup = BeautifulSoup(html_content, 'html.parser')
        product_cards = soup.find_all('x-cell')
...
collection_urls = [
    {'category': 'men-sneakers', 'url': 'https://gullylabs.com/collections/mens-sneakers'},
